...
class Moto(models.Model):
    ...
    def get_absolute_url(self): 
        return reverse('moto_detalle', kwargs={'pk': self.patente})
    def motorista_actual(self):
        asignacion = AsignacionMoto.objects.filter(
            moto=self,
            fechaTermino__isnull=True
        ).order_by('-fechaAsignacion').first()

        return asignacion.motorista if asignacion else None
